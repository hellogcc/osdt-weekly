# OSDT Weekly 2026-07-29 第369期

点击「查看原文」跳转到 GitHub 上对应文件，链接就可以点击了。或直接点击：
https://github.com/hellogcc/osdt-weekly/tree/master/weekly-2026

## 近期活动预告

## 编译社区的八卦信息

### GCC

- [patch] AVR: Add 64-bit fixed-point divisions to libgcc
  https://gcc.gnu.org/pipermail/gcc-patches/2026-July/725204.html

- [PATCH v3 0/7][X86]: AVX10_V2_AUX ISA support.
  https://gcc.gnu.org/pipermail/gcc-patches/2026-July/725365.html

- [PATCH v1 0/4] Support RVV register overlap for vwsub[u].wv
  https://gcc.gnu.org/pipermail/gcc-patches/2026-July/725447.html

- [PATCH v4 1/5] RISC-V: Improve macro-fusion dump output and comments
  https://gcc.gnu.org/pipermail/gcc-patches/2026-July/725608.html

- [PATCH v3 0/3] ivopt: Changes regarding reg pressure cost.
  https://gcc.gnu.org/pipermail/gcc-patches/2026-July/725698.html

- [patch][vect]: block versioning for VLA when alignment can never be reached [PR126369]
  https://gcc.gnu.org/pipermail/gcc-patches/2026-July/725713.html

### BINUTILS

- [PATCH v2 0/6] Add support for nanoMIPS architecture
  https://sourceware.org/pipermail/binutils/2026-July/150409.html

- [PATCH v4 00/12] RISC-V: assorted fixes and (hopefully) improvements
  https://sourceware.org/pipermail/binutils/2026-July/150390.html

- [PATCH] x86-64: Generate R_X86_64_PLT32 for ".long foo@PLT - .L4"
  https://sourceware.org/pipermail/binutils/2026-July/150429.html

- RFC: Should the linker warn about and/or control the propagation of audit libraries ?
  https://sourceware.org/pipermail/binutils/2026-July/150507.html

- [PATCH v4] RISC-V: Add new relocation types for global array accesses with non-constant indices
  https://sourceware.org/pipermail/binutils/2026-July/150537.html

### GDB

- [PATCH 00/27] Teach the testsuite about the Windows/MSVC target
  https://sourceware.org/pipermail/gdb-patches/2026-July/228904.html

- [PATCH 0/7] MIPS+testsuite: BLEZL stepping fix and associated test infrastructure
  https://sourceware.org/pipermail/gdb-patches/2026-July/228944.html

- [PATCH v1 0/6] gdb: introduce file_reader_t to read procfs files
  https://sourceware.org/pipermail/gdb-patches/2026-July/229053.html

### GLIBC

- Suggestion: Changes to release process, release from branch
  https://sourceware.org/pipermail/libc-alpha/2026-July/179165.html

- [PATCH v11 0/5] elf: Allow RPATH/RUNPATH for static-pie
  https://sourceware.org/pipermail/libc-alpha/2026-July/179232.html

### LLVM/Clang/LLDB/LLD

本节内容来自 [LLVM Weekly 第 656 期](http://llvmweekly.org/issue/656)，
[Alex Bradbury](https://www.linkedin.com/in/alex-bradbury/) 大哥持续稳定输出。

* 太需要了： Bojun Seo [proposed adding a DoubleFreeSanitizer](https://discourse.llvm.org/t/rfc-introduction-of-doublefreesanitizer-dsan/91363).

* 轻量级插桩是个经久不衰的工程努力： Tatiana Larina started an RFC thread on [adding lightweight instrumentation](https://discourse.llvm.org/t/rfc-lightweight-callback-based-lowering-for-fprofile-instr-generate-site-selection/91367), " a new Clang/LLVM instrumentation mode that reuses the frontend site selection from -fprofile-instr-generate but lowers instrumentation sites to a user-defined callback instead of inline counters."

* 会有些性能提升？ A scalar replacement of aggregates pass implementation for logical pointers (i.e. those created by llvm.structured.gep and llvm.structured.alloca) was implemented.
  [03b5c52](https://github.com/llvm/llvm-project/commit/03b5c5285eaf).

* 日进跬步： The RISC-V backend now always enables memcmp/bcmp expansion for pointers known to be aligned.
  [ca276b4](https://github.com/llvm/llvm-project/commit/ca276b4822b1).

* RISC-V添加了么： Initial documentation was added on adding a new BOLT backend.
  [a822836](https://github.com/llvm/llvm-project/commit/a8228369cae4).

### QEMU

- [PATCH 0/2] Add Model for Tenstorrent Atlantis PRCM
  https://lists.nongnu.org/archive/html/qemu-riscv/2026-07/msg01136.html

- [PATCH v1 0/3] hw/riscv: add SDHCI support for K230
  https://lists.nongnu.org/archive/html/qemu-riscv/2026-07/msg01131.html

- [PATCH] hw/timer: add k230 dwapb timer
  https://lists.nongnu.org/archive/html/qemu-riscv/2026-07/msg01073.html

- [PATCH 00/11] hw/riscv: Add K230 SPI, QSPI, IDMA and XIP support
  https://lists.nongnu.org/archive/html/qemu-riscv/2026-07/msg01056.html

- [PATCH v8 00/11] target/riscv: Implement Smsdid and Smmpt extension
  https://lists.nongnu.org/archive/html/qemu-riscv/2026-07/msg01018.html

- [PATCH 00/26] hw/riscv: Restore Microchip PolarFire SoC Icicle Kit firmware boot
  https://lists.nongnu.org/archive/html/qemu-riscv/2026-07/msg00992.html

- [PATCH v2 00/50] Introduce helper-to-tcg
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg08226.html

- [PATCH 00/25] hw/arm: Facebook SanMiguel BMC and TMP75-family sensors
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg08105.html

- [PATCH v20 00/15] virtio-net: live-TAP local migration
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg08011.html

- [RFC v2 00/24] Add Realm support to QEMU-VMM
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg07914.html

- [PATCH v5 0/6] vhost-user-blk: add compatibility with older qemu versions
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg07704.html

- [PATCH 0/3] accel/mshv: add gdbstub guest debugging support
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg07397.html

- [RFC PATCH 00/11] igb: Add experimental VF live migration support
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg07218.html

- [RFC PATCH v1 0/5] hw/gpio: STM32F1xx GPIO controller in Rust
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg07091.html

- [PATCH 0/5] hw/arm/raspi4b: RNG200 and thermal sensor
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06945.html

- [PATCH 00/19] hw/arm/raspi4b: working PCIe and GENET (real networking)
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06940.html

- [PATCH 0/2] target/i386: Add AMD LBR Extension v2 support
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06911.html

- [PATCH 0/6] target/arm: Implement FEAT_IDTE3
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06680.html

- [PATCH v4 00/15] s390x/pci: Implement migration for emulated devices
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06664.html

- [PATCH 0/6] target/arm: Implement FEAT_FGWTE3
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06658.html

- [PATCH 00/27] single-binary: implement dynamic filtering for machine types
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06625.html

- [PATCH 0/4] arm: add Cortex-M85, SSE-310, and mps3-an555
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06620.html

- [PATCH RFC 00/15] vhost-user: isolated memory
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06603.html

- [PATCH 0/2] target/i386: Add missing features for Hygon Dhyana and support for Dharma
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06482.html

- [PATCH for-11.2 0/3] accel/tcg: Add infrastructure for atomic 64-byte load/store
  https://lists.nongnu.org/archive/html/qemu-devel/2026-07/msg06322.html

### RISC-V in China

- https://ruyisdk.cn 每日八卦都在这里了。

### RT-Thread 社区

- 风平浪静。招募小编！

### AI Compilers

- 风平浪静。招募小编！

### 本周工具链岗位

**往期社招及实习生招聘长期有效。**

### 本周推荐阅读

本周推荐：《AI未来已来》

作者：李开复

推荐：小编1号

强烈推荐所有人都阅读李开复老师的这本书，有非常贴近现实的思考和冲击案例，也面向不同的人群给出了清晰的建议。我已经开始用起来了书中的一些方法论（实际上也偷懒直接用了里面的一些个人经验照搬）。现在我在AI快速发展的当下，更加自信一点了，并且充了500块钱DeepSeek开始新一轮折腾。
